          ...
          {clsRows.map((row,i)=>(
            <div key={i} style={{background:"#F9FAFB",borderRadius:10,padding:"1rem",marginBottom:"0.75rem",border:"0.5px solid #E5E7EB"}}>
              ...
              <Row3>
                ...
                <Field label="Type"><select style={sel} value={row.type} onChange={e=>setClsRows(p=>p.map((r,x)=>x===i?{...r,type:e.target.value}:r))}>{["Classroom","Lab","Office","Storage","Science Laboratory","Library","Hospitality Room","Computer Lab","Tuck Shop","Consumer Room","Clothing Store"].map(v=><option key={v}>{v}</option>)}</select></Field>
